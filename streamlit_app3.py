...
DB_URI = "postgresql+pg8000://neondb_owner:npg_4avLdFYQJqt1@ep-sweet-bonus-acwjutwf-pooler.sa-east-1.aws.neon.tech/neondb"
...
st.markdown("Integrated Bishop Stability Model with Neon Cloud Monitoring")
...
def plot_fs_gauge(fs_value): # !!!
    fig = go.Figure(go.Indicator(
        ...
        gauge = {
            ...
            'bordercolor': "gray",
            'steps': [
                {'range': [0, 1.2], 'color': '#ff4b4b'}, # Red
                {'range': [1.2, 1.5], 'color': '#ffa500'}, # Orange/Yellow
                {'range': [1.5, 2.5], 'color': '#00cc96'}  # Green
            ],
            'threshold': {
...
if not data.empty:
    # --- SIDEBAR CONTROLS (Common to both tabs) ---
    ...
    with tab1:
        ...
        # Physics Run
        fs, slices, water_line = calculate_slope_stability(xc, yc, R, u_latest)

        col1, col2 = st.columns([1, 3])
                
        with col1:
            if fs:
                # Add the Gauge Chart here
                st.plotly_chart(plot_fs_gauge(fs), use_container_width=True)
                #st.metric(label="Factor of Safety", value=fs)
                if fs < 1.0: st.error("CRITICAL: FAILURE")
                elif fs < 1.2: st.error("WARNING: UNSTABLE")
                elif fs < 1.5: st.warning("ALARM: BELOW SPEC")
                else: st.success("STABLE")
            else:
                st.error("No Intersection")

            st.write(f"**Sensor:** {u_latest} kPa")
            ...
            if slices:
                for s in slices:
                    ax.bar(s['x_mid'], s['h'], width=s['b'], bottom=s['y_bot'], 
                           color='orange', alpha=0.5, edgecolor='black', linewidth=0.3)
        ...
        if st.button("🚀 Start Global Scan"):
            ...
            for i, py in enumerate(grid_y):
                for j, px in enumerate(grid_x):
                    val, _, _ = calculate_slope_stability(px, py, R, u_latest)
            ...
            sns.heatmap(fs_matrix, annot=True, fmt=".2f", cmap="RdYlGn", 
                        xticklabels=np.round(grid_x, 0), yticklabels=np.round(grid_y, 0),
                        ax=ax_h, cbar_kws={'label': 'Factor of Safety'})
            ax_h.invert_yaxis()
    ...
    st.warning("Database empty for April 6. Please run your insertion script.")